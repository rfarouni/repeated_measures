...
The next three question will involve working with the following data:
...
Compute  the vector of estimated  parameters $\hat{\boldsymbol \beta}$ for the simple regression problem give the data provided above.
...
Now play with the interactive visualization [here](http://setosa.io/ev/ordinary-least-squares-regression/) and try to figure out what kind of tranformation $\hat{\mathbf{H}}$ accomplished. The answer is one word only. Think about the dimension of $\mathbf{H}$ with relation to the dimension of the data vector $\mathbf{y}$  
