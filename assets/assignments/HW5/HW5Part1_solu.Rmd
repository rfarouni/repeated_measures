...
\end{bmatrix}
$$
That is, the covariance matrix is unconstrained. For incremental fit indicies, the null hypothesis is tested against the null/independence model that the observed variables are uncorrelated. In this case, we have
...
H_1:\boldsymbol \Sigma= \begin{bmatrix}
  \sigma_{11} & 0  & \ldots & 0  \\
   0 & \sigma_{22}  & \ldots & 0  \\
 \vdots &\vdots &  \ddots  & \vdots \\
0 & \ldots  & 0 &\sigma_{10,10}
\end{bmatrix}
$$
The fit indices show an almost perfect fit to the data. **We fail to reject the null hypothesis**. There is evidence that the  sample covariance matrix of our data can have the decomposition implied by the null hypothesis. 
